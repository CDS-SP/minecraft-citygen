#!/usr/bin/env python
"""Regenerate the Minecraft release table from server JARs.
...
and reads each jar's DataVersion from its bundled ``version.json``.
...
guaranteed to exist in the target and no per-block version map is needed -- just
the release table for labelling and target resolution.

Requirements and behaviour:

  - downloaded server JARs are cached under ``tools/`` and are git-ignored
  - a version whose download fails is skipped with a warning, so a
    partial-but-valid table is still written; the summary lists what failed
"""
...
USER_AGENT = "CityGen version-table updater"
...
def main() -> int:
    ...
    for meta in metas:
        ...
        print(f"  {name}: DataVersion {data_version}")

    if not releases:
        print("error: no releases were read; nothing written.", file=sys.stderr)
