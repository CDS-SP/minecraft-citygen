...
def _schem_file(width, height, length, palette, data, data_version):
    blocks = Compound({
...
def sponge_schem_from_cells(cells, data_version):
    ...
    file = _schem_file(width, height, length, palette, signed, data_version)
    return file, palette


def sponge_schem_from_grid(grid, palette, data_version):
    height, length, width = grid.shape
    data = encode_varint_array(grid.reshape(-1))
    return _schem_file(width, height, length, palette, data, data_version)
...
def write_sponge_schem_cells(cells, path, data_version):
    file, palette = sponge_schem_from_cells(cells, data_version)
    save_sponge_schem(file, path)
    return palette


def write_sponge_schem_grid(grid, palette, path, data_version):
    file = sponge_schem_from_grid(grid, palette, data_version)
    save_sponge_schem(file, path)
